 ...
 * for PreToolUse (`allow` / `deny` / `ask` / `defer`), and is verified end to
 * end against a real runtime in test/contract/host-integration/.
 *
...
function outputAsk(reason, alternatives) {
  ...
  if (isCursorRuntime()) {
    // Cursor has no `ask` tier; the closest honest mapping is to surface the
    // reason to the agent and let the host's own confirmation flow run.
    console.log(JSON.stringify({ permission: 'ask', user_message: reason, agent_message: detail }));
    process.exit(0);
    return;
  }
...
function installCrashRecorder(event) {
  ...
  process.on('uncaughtException', (err) => {
    record('threw', err);
    // Preserve Node's default: an uncaught exception is fatal.
    process.exit(1);
  });
...
/**
 * stdin에서 JSON 동기적 읽기 + dispatch 기록.
 * @returns {*}
 */
